...
is_num "${VAD_THRESHOLD:-}" || err "VAD_THRESHOLD must be numeric (0..1)"
is_num "${VAD_MIN_SILENCE_MS:-}" || err "VAD_MIN_SILENCE_MS must be numeric (ms)"
is_num "${WLK_CHUNK:-}" || err "WLK_CHUNK must be numeric (seconds)"
is_int "${VOSK_TIMEOUT:-}" || err "VOSK_TIMEOUT must be an integer (seconds)"
is_int "${WLK_READY_TIMEOUT:-}" || err "WLK_READY_TIMEOUT must be an integer (seconds)"
